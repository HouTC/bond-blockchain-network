...
async function couponPayment(cp) {
    ...
    var properCoupon = cp.bond.issueAmount * cp.bond.interestRate / 4;
    ...
    if (cp.bond.pStatus === false){
        ...
        if (cp.coupon === properCoupon &&  sum <= cp.bond.issueAmount){
            // Update the participant with the new value.
            cp.issuer.balance -= cp.coupon;
            ...
            let event = getFactory().newEvent('org.example.mynetwork', 'PaymentIncorrectEvent');
            event.issuer = cp.issuer;
            emit(event);
        }
    }else{
...
async function distributeCoupon(dc) {
    ...
    */
    ...
    if(dc.bond.pStatus == true){
        ...
        dc.bond.pStatus = false;
        // Get the participant registry for the participant.
        const assetRegistry = await getAssetRegistry('org.example.mynetwork.Bond');
        // Update the asset in the asset registry.
        await assetRegistry.update(dc.bond);

...
async function makePayment(mp) {
    ...
    var bulkCoupon = sum * bond.interestRate / 4;
    ...
    if (bond.nextPaymentDate < bond.maturityDate && bond.matured == false){
        ...
        // declare coupon variable
            ...
            for(var i = 0; i < bond.holdings.length; i++){
                ...
                // emit PrincipalReceivedEvent event
                let event = getFactory().newEvent('org.example.mynetwork', 'PrincipalReceivedEvent');
                event.investor = bond.holdings[i].owner;
                emit(event);
            }
            // emit PrincipalRepaidEvent event
            let event = getFactory().newEvent('org.example.mynetwork', 'PrincipalRepaidEvent');
            event.issuer = bond.issuer;
            emit(event);
    }
        ...
        throw new Error('The bond has already matured.');
    }
}
...
async function setupDemo(sd) {
    ...
    issuer1.firstListed = '24 Jul 2002';
    issuer1.balance = 100000000;
    ...
    const investor3 = factory.newResource(namespace, 'Investor', 'charlie');
    investor3.name = 'Charlie';
    investor3.balance = 0;
    await investorRegistry.add(investor3);

    const investor4 = factory.newResource(namespace, 'Investor', 'dave');
    investor4.name = 'Dave';
    investor4.balance = 0;
    await investorRegistry.add(investor4);
    ...
    holding1.bond = '1';
    holding1.faceValue = 500000;
    await holdingRegistry.add(holding1);

    const holding2 = factory.newResource(namespace, 'Holding', '2');
    holding2.owner = investor2;
    holding2.bond = '1';
    holding2.faceValue = 500000;
    await holdingRegistry.add(holding2);

    const holding3 = factory.newResource(namespace, 'Holding', '3');
    holding3.owner = investor3;
    holding3.bond = '2';
    holding3.faceValue = 500000;
    await holdingRegistry.add(holding3);

    const holding4 = factory.newResource(namespace, 'Holding', '4');
    holding4.owner = investor4;
    holding4.bond = '2';
    holding4.faceValue = 500000;
    await holdingRegistry.add(holding4);
    ...
    bond1.minHolding = 5000;
    bond1.type = 'Debt/Equity Hybrids';
    await bondRegistry.add(bond1);

    const bond2 = factory.newResource(namespace, 'Bond', '2');
    bond2.issuer = issuer1;
    bond2.issueAmount = 1500000;
    bond2.interestRate = 0.04;
    bond2.issueDate = new Date("2018-06-22");
    bond2.interestPaymentFreq = 'Quarterly';
    bond2.maturityDate = new Date("2019-06-22");
    bond2.nextPaymentDate = new Date("2018-09-22");
    bond2.holdings = [holding3, holding4];
    bond2.minHolding = 5000;
    bond2.type = 'Debt/Equity Hybrids';
    await bondRegistry.add(bond2);

    // create the regulator
    const regulatorRegistry = await getParticipantRegistry(namespace + '.Regulator');
    const regulator1 = factory.newResource(namespace, 'Regulator', 'ird');
    regulator1.name = 'Inland Revenue Department';
